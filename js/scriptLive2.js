...
let currentPlayer = 1;
actualitzarVariable("parametres/currentPlayer",1)
let descompte = true;
actualitzarVariable("parametres/descompte",true)
let jug1 = true;
...
const startTimer = () => {
  playing = true;
  actualitzarVariable("parametres/playing",true)
      ...
      if (playing && jug1) {
        ...
        if (p1sec === 0) {
          ...
          if (p1sec === 0 && p1time.minutes === 0) {
            ...
            //playing = false;
            jug1 = false;
            actualitzarVariable("parametres/jug1",false)
            if (descompte) {
              tempsDescompte();
            }
          }
      ...
      if (playing && jug2) {
        ...
        if (p2sec === 0) {
          ...
          if (p2sec === 0 && p2time.minutes === 0) {
            ...
            //playing = false;
            jug2 = false;
            actualitzarVariable("parametres/jug2",false)
            if (descompte) {
              tempsDescompte();
            }
          }
...
function canvitorn(jug) {
  ...
  if (!playing && botoStart.textContent === "CONTINUA") {
    currentPlayer = jug;
    actualitzarVariable("parametres/currentPlayer",jug)
...
tempsBtn.addEventListener("click", () => {
  ...
  incrementar("jugador1","min",temps)
  incrementar("jugador1","sec",0)
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  jug1 = temps1.penal;
  actualitzarVariable("parametres/jug1",temps1.penal)
  ...
  function activapenal1() {
    ...
    document.getElementById("penal1").textContent =
      ...
      incrementar("jugador1","penal", "-" + (temps1.minutes + 1) * 10)
  }
  ...
  jug2 = temps2.penal;
  actualitzarVariable("parametres/jug2",temps2.penal)
  ...
  function activapenal2() {
    ...
    document.getElementById("penal2").textContent =
      ...
      incrementar("jugador2","penal", "-" + (temps2.minutes + 1) * 10)
  }
});
